...
def _fake_graph_data(num_nodes: int = 5, num_edges: int = 4, depth: int = 2) -> Dict[str, Any]:
...
class TestHistogramVisualizationConfig:
    ...
    def test_default_config_creation(self) -> None:
        ...
        assert cfg.dataset_hf_identifier == "helena-balabin/vg_coco_graphs_merged"
...
class TestLoadDatasetWithProperties:
    ...
    @patch("relationalstructureinclip.visualization.histogram_graph_properties.load_dataset")
    def test_load_dataset_success(self, mock_load_dataset: Any) -> None:
        ...
        cfg = HistogramVisualizationConfig(
            dataset_cache_dir="/tmp/test_cache",
    ...
    @patch("relationalstructureinclip.visualization.histogram_graph_properties.load_dataset")
    def test_missing_graph_columns_raises_error(self, mock_load_dataset: Any) -> None:
        """Raise ValueError when required graph columns are missing."""
        # Create dataset without required columns
        mock_dataset = Dataset.from_dict({"other_column": [1, 2, 3]})
        mock_load_dataset.return_value = mock_dataset
        
        cfg = HistogramVisualizationConfig()
        
        with pytest.raises(ValueError, match="Missing graph columns in dataset"):
            load_dataset_with_properties(cfg)
...
class TestIntegration:
    ...
    def test_end_to_end_workflow(self, mock_load_dataset: Any) -> None:
        ...
        expected_subplots = len(cfg.text_graph_columns + cfg.image_graph_columns) * len(cfg.properties)
    ...
    def test_config_validation(self) -> None:
        ...
        assert len(cfg.text_graph_columns + cfg.image_graph_columns) > 0  # Has graph types