...
class VisualizationConfig:
    ...
    graph_type_labels: Dict[str, str] = field(
        default_factory=lambda: {
            "amr_graphs": "AMR",
    ...
    text_graph_types: List[str] = field(
        default_factory=lambda: ["amr_graphs", "dependency_graphs"]
    )
    image_graph_types: List[str] = field(
        default_factory=lambda: [
            "image_graphs",
            "action_image_graphs",
            "spatial_image_graphs",
    ...
    separate_property_plots: bool = (
        True  # also produce separate text/image figures
    )
...
def load_probe_results(file_path: str | Path) -> pd.DataFrame:
    ...
    logger.info(
        "Loaded %d rows with %d unique models and %d graph types",
        len(df),
        df["model"].nunique(),
        df["graph_type"].nunique(),
    )
...
def prepare_visualization_data(
    df: pd.DataFrame, cfg: VisualizationConfig
    ...
    if cfg.add_average_plot:
        ...
        if metric_value_cols:
            ...
            logger.warning(
                "No metric columns found to compute average; skipping average plot."
            )
    ...
    for metric_name, metric_cfg in metrics_to_use.items():
        ...
        if column not in df.columns:
            logger.warning(
                "Metric column '%s' not found in data. Skipping %s.",
                column,
                metric_name,
            )
            continue
        ...
        if metric_name == "average" and cfg.add_average_plot:
            ...
            group_map.update(
                {gt: "Image (all)" for gt in cfg.image_graph_types}
            )
            df_avg = df.copy()
            ...
            logger.info(
                "Prepared Text/Image average plot with shape: %s",
                mean_pivot_df.shape,
            )
            continue
        ...
        available_graph_types = [
            gt for gt in cfg.graph_types if gt in mean_pivot_df.index
        ]
        ...
        if cfg.error_bars and std_column and std_column in df.columns:
            ...
            logger.info(
                "Prepared data for %s with error bars: %s",
                metric_name,
                mean_pivot_df.shape,
            )
        else:
            logger.info(
                "Prepared data for %s without error bars: %s",
                metric_name,
                mean_pivot_df.shape,
            )
...
def create_grouped_bar_plots(
    ...
    for i, (metric_name, metric_data) in enumerate(data.items()):
        ...
        for j, model in enumerate(mean_df.columns):
            ...
            if (
                cfg.error_bars
                and std_df is not None
                and model in std_df.columns
            ):
        ...
        # Customize subplot
        ax.set_title(
            metric_cfg.get("title", metric_name), fontsize=cfg.title_fontsize
        )
        ax.set_ylabel(
            metric_cfg.get("ylabel", ""), fontsize=cfg.axis_label_fontsize
        )
        ax.set_xlabel("Graph Type", fontsize=cfg.axis_label_fontsize)

        # Set x-axis labels using graph type labels from config
        graph_type_labels = [
            cfg.graph_type_labels.get(gt, gt) for gt in mean_df.index
        ]
        ax.set_xticks(x_pos)
        ax.set_xticklabels(
            graph_type_labels,
        ...
        ax.tick_params(
            axis="y",
            which="major",
            labelsize=(cfg.y_tick_label_fontsize or cfg.tick_label_fontsize),
        )
        ...
        if metric_cfg.get("ylim") is not None:
            ...
            metric_name in metric_names_for_common and common_ylim is not None
        ):
    ...
    handles_labels = [
        (h, l) for h, l in zip(handles, labels) if not l.startswith("_")
    ]
    if handles_labels:
...
def main(cfg: DictConfig) -> None:  # pragma: no cover - thin wrapper
    ...
    if not prepared_data:
        logger.error(
            "No valid data to visualize. Check input file and metric columns."
        )
        return
    ...
    if structured_cfg.separate_property_plots:
        ...
        ext = (
            Path(structured_cfg.output_filename).suffix
            or f".{structured_cfg.output_format}"
        )
        ...
        for subset_name, subset_types in subsets:
            ...
            if df_subset.empty:
                logger.warning(
                    "No data for %s subset (%s). Skipping.",
                    subset_name,
                    subset_types,
                )
                continue
            ...
            if not prepared_subset:
                logger.warning(
                    "No valid data to visualize for %s subset.", subset_name
                )
                continue
            ...
            logger.info(
                "Saved %s-only visualization to: %s", subset_name, out_path
            )
