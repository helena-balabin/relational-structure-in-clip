...
def _compute_clip_embeddings(
    model_id, texts, image_paths, model_cache_dir, batch_size=64
):
    ...
    processor = AutoProcessor.from_pretrained(
        (
            ...
            if "graph-clip" in model_id
            else model_id
        ),
        cache_dir=model_cache_dir,
        use_fast=True,
    )
    model = AutoModel.from_pretrained(model_id, cache_dir=model_cache_dir).to(
        device
    )
    model.eval()
    ...
    with ThreadPoolExecutor() as executor:
        for i in tqdm(
            ...
            desc=f"Computing embeddings for {model_id}",
        ):
    ...
    for graph_col in graph_columns:
        ...
        for task in tasks:
            ...
            if len(filtered_labels) == 0:
                logger.warning(
                    "No valid examples for task %s on %s, skipping",
                    task.target,
                    graph_col,
                )
                continue
            ...
            for fold in range(n_cv_folds):
                ...
                splitter_fold = DataSplitter(
                    train_ratio=train_ratio, seed=fold_seed
                )
            ...
            for metric_name, value in averaged_metrics.items():
                mlflow.log_metric(
                    f"{graph_col}_{task.target}_{metric_name}", value
                )
                mlflow.log_metric(
                    f"{graph_col}_{task.target}_{metric_name}_std",
                    std_metrics[metric_name],
                )

            # Store averaged result
            result = task.create_result(
                model_id, graph_col, averaged_metrics, std_metrics
            )
...
def main(cfg: DictConfig):
    ...
    with mlflow.start_run(run_name=f"probe_run_{cfg.get('random_seed', 42)}"):
        ...
        df["image_path"] = df["filepath"].apply(
            lambda x: str(coco_base_dir / x)
        )
        ...
        mlflow.log_metrics(
            {
                ...
                "total_graph_columns": len(text_graph_columns)
                + len(image_graph_columns),
            }
        )
        ...
        if not text_graph_columns and not image_graph_columns:
            ...
            all_graph_columns = [
                col for col in df.columns if col.endswith("_graphs")
            ]
        ...
        trainer = ProbeTrainer(
            ...
            alpha_range_and_samples=tuple(
                cfg.get("alpha_range_and_samples", (-3, 3, 7))
            ),
        )
        ...
        for model_id in models:
            ...
            with mlflow.start_run(
                run_name=f"model_{model_id.replace('/', '_')}", nested=True
            ):
                mlflow.log_param("model_id", model_id)

                logger.info(
                    "Computing separate text and image embeddings for %s",
                    model_id,
                )
                ...
                "num_models": (
                    len(
                        [
                            r
                            for r in results
                            if r["graph_type"] == all_graph_columns[0]
                            and r["target"] == "num_nodes"
                        ]
                    )
                    if results and all_graph_columns
                    else 0
                ),
